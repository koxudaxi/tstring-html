...
Updates all occurrences of package version pins like
`html-tstring==X.Y.Z`, `thtml-tstring==X.Y.Z`, etc.
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Update package version in docs")
    parser.add_argument(
    ...
    target_files = [f for f in DOCS_DIR.rglob("*.md") if f.name not in EXCLUDE_FILES]
