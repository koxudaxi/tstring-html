"""llms.txt generator for tstring-html documentation.
...
OPTIONAL_PATTERNS = ("development/setup.md",)
SKIP_PREFIXES = ("```", "!!!", "[![", "![", "<", "---")
    ...
    for item in nav:
        for title, value in item.items():
            if isinstance(value, str):
                ...
                sections.append(
                    NavSection(
                        title=title, children=flatten_nav(value, depth + 1), depth=depth
                    )
                )
    return sections
...
def extract_page_info(md_path: Path, url: str, depth: int = 0) -> PageInfo | None:
    ...
    title, desc_lines, in_code, found_h1 = (
...
def generate_llms_txt(
    ...
    main, optional = [], []
    for s in sections:
        is_opt = (s.path and is_optional(s.path)) or (
            s.children and all(is_optional(c.path) for c in s.children)
        )
...
def generate_llms_full_txt(pages: list[PageInfo]) -> str:
    ...
    for page in pages:
        ...
        parts.extend(
            [f"# {page.title}", "", f"Source: {page.url}", "", content, "", "---", ""]
        )
    return "\n".join(parts)
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Generate llms.txt files from documentation"
    )
    parser.add_argument(
        "--check",
        action="store_true",
