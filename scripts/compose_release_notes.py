...
def render_release_notes(generated_notes: str, breaking_pr_number: int | None) -> str:
    lines = [
        ...
        (
            "Keep the required package headings unchanged so validation and "
            "changelog sync can parse the release body."
        ),
        "-->",
        "",
        "## Highlights",
    ...
    if breaking_pr_number is not None:
        lines.extend(
            [
                ...
                "",
                (
                    f"- Review PR #{breaking_pr_number} before publishing. "
                    "It is labeled `breaking-change`."
                ),
                "",
            ]
        )
